...
export class TxoStore {
  ...
  async ingest(
    ...
    for (const input of tx.inputs) {
      ...
      if (input.sourceTransaction) {
        ...
        input.sourceTransaction = await this.stores.txns!.loadTx(input.sourceTXID, fromRemote);
        if (!input.sourceTransaction) throw new Error(`Failed to get source tx ${input.sourceTXID}`);
      }
    }

    ctx.spends = (await this.storage.getMany(
      tx.inputs.map((i) => new Outpoint(`${i.sourceTXID}_${i.sourceOutputIndex}`))
    )).map((spend, vin) => {
      if (!spend) {
        const input = tx.inputs[vin];
        const spentOutput = input.sourceTransaction!.outputs[input.sourceOutputIndex];
        spend = Txo.fromObject(spend, this.indexers) || new Txo(
          new Outpoint(tx.inputs[vin].sourceTXID!, tx.inputs[vin].sourceOutputIndex),
    ...
    ctx.txos = (await this.storage.getMany(
    ...
    for (const txo of ctx.txos) {
      for (const [tag, idxData] of Object.entries(txo.data)) {
        for (const e of idxData.events) {
          this.events?.emit(`evt:${tag}:${e.id}`, e.value, txo.outpoint.toString());
        }
      }
    }
    return ctx;
  ...
  async syncTxLogs() {
    ...
    for (const latestLog of latestLogs) {
      ...
      const newLogs = await this.services.inv.pollTxLogs(latestLog.owner, latestLog.height || 0);
      ...
      await this.storage.putIngests(puts.map(log => new Ingest(log.txid, log.height, log.idx | 0, false)));
